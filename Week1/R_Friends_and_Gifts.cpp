...
int main()
{
    ...
    vector<int> vt(n), vt1;
    set<int> st;
    vector<int> positions_to_be_filled;
    for (int i = 1; i <= n; ++i)
    {
        st.insert(i);
    }
    for (int i = 0; i < n; ++i)
    {
        ...
        if (vt[i] == 0)
        {
            positions_to_be_filled.push_back(i);
        }
        else
        {
            st.erase(vt[i]);
    ...
    for (int i = 0; i < positions_to_be_filled.size(); ++i)
    {
        vt[positions_to_be_filled[i]] = numbers[i];
    }
    int index = 0;
    for (int i = 0; i < n; ++i)
    {
        if (vt[i] == (i + 1))
        {
            if (index - 1 >= 0)
            {
                swap(vt[i], vt[positions_to_be_filled[index - 1]]);
            }
            else
            {
                swap(vt[i], vt[positions_to_be_filled[index + 1]]);
